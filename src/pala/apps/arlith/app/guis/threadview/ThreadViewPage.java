...
import static pala.apps.arlith.app.guis.threadview.ThreadFormattingUtils.render;
...
import pala.apps.arlith.backend.graphics.dialogs.SimpleDialog;
import pala.apps.arlith.backend.graphics.nodes.AdvancedColorPicker;
import pala.apps.arlith.backend.graphics.nodes.StylePicker;
...
public class ThreadViewPage extends BindHandlerPage {
	...
	public void show(ArlithWindow win) throws WindowLoadFailureException {
		...
		sp.getColorPicker().setColor(Arlith.getBaseColor());
		...
		sp.setOnSelect(() -> {
			...
			if (!Arlith.getBaseColor().equals(sp.getColor())) {
