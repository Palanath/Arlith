package pala.apps.arlith.frontend.guis.home.center.friendstab;
...
public class FriendListView extends TableView<ClientUser> {
	...
	private static final Border SELECTED_ROW_BORDER = FXTools.getBorderFromColor(JFXArlithRuntime.DEFAULT_ACTIVE_COLOR, 1),
			HOVER_ROW_BORDER = FXTools.getBorderFromColor(JFXArlithRuntime.DEFAULT_BASE_COLOR, 1);
			...
			TableRow<ClientUser> r = new TableRow<ClientUser>() {
				...
				private void responsivelyStyle() {
					BORDER_HANDLER: if (!isEmpty()) {
						if (isSelected()) {
							setTextFill(JFXArlithRuntime.DEFAULT_ACTIVE_COLOR);
							setPadding(Insets.EMPTY);
							setBorder(SELECTED_ROW_BORDER);
						} else if (isHover()) {
							setTextFill(JFXArlithRuntime.DEFAULT_BASE_COLOR);
							setPadding(Insets.EMPTY);
				...
				public void style(ClientUser value) {
					if (value == null)
						...
						try {
							...
							GUIUtils.getGuiLogger().err("An error occurred while loading a user's icon.");
			...
			public void style(ClientUser value) {
				if (value == null)
					...
					try {
						...
						GUIUtils.getGuiLogger().err("An error occurred while loading a user's tag.");
						GUIUtils.getGuiLogger().err(e);
						setText("!#ERR");
					}
			}
				...
				public void style(ClientUser value) {
					if (value == null) {
						...
						try {
								...
								if (value.client().getOutgoingFriendRequests().contains(value)) {// Show "rescind"
									...
									rescind.setOnAction(new EventHandler<ActionEvent>() {
										...
										public void handle(ActionEvent event) {
											...
											GUIUtils.getGuiLogger().err("Not yet implemented... :-)");
										}
									});
									...
									accept.setOnAction(event -> {
										try {
											...
											GUIUtils.getGuiLogger().err("An error occurred while trying to friend request the user.");
											GUIUtils.getGuiLogger().err(e);
							...
							GUIUtils.getGuiLogger().err("An error occurred while checking the friend status of another user.");
							GUIUtils.getGuiLogger().err(e);
							setText("Error");
