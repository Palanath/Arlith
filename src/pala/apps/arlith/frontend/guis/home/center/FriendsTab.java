...
public class FriendsTab extends PanelTab {
			...
			if (htind != -1) {
				...
				if (name.isEmpty() || disc.isEmpty())
					...
					try {
						...
						GUIUtils.getGuiLogger().err("Failed to check if user being friend requested is already friended.");
						GUIUtils.getGuiLogger().err(e);
				...
				RESOLVE_FRIEND_STATE: {
					try {
						...
						GUIUtils.getGuiLogger().err("Failed to check friend status of the user being friend requested.");
						GUIUtils.getGuiLogger().err(e);
					}
...

	{
		...
		try {
			...
			GUIUtils.getGuiLogger().err("Failed to list incoming friend requests.");
			GUIUtils.getGuiLogger().err(e);
		}
		try {
			for (ClientUser u : home.getApp().getClient().listFriends())
		...
		try {
			...
			GUIUtils.getGuiLogger().err("Failed to list outgoing friend requests.");
			GUIUtils.getGuiLogger().err(e);
		}
	}
