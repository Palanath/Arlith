...
public class ThreadViewPage extends BindHandlerPage {
	...
	private HBox renderHeader(ClientMessage msg) {
		...
		try {
			...
			ArlithFrontend.getGuiLogger().err("Failed to get the name of the author of a message.");
			ArlithFrontend.getGuiLogger().err(e);
			ArlithFrontend.getGuiLogger().err("Message content: " + msg.getText());
		...
		try {
			...
			ArlithFrontend.getGuiLogger().err("Failed to obtain the profile picture for the author of a message.");
			ArlithFrontend.getGuiLogger().err(e);
	...
	public void show(ArlithWindow win) throws WindowLoadFailureException {
		...
		try {
			...
			ArlithFrontend.getGuiLogger().err("Failed to load the CENTER PANEL of the Thread Viewer window.");
		...
		synchronized (thread) {
			...
			try {
				...
				ArlithFrontend.getGuiLogger().err("An error occurred while loading messages from a thread.");
				ArlithFrontend.getGuiLogger().err(e);
		...
		try {
			...
			ArlithFrontend.getGuiLogger().err("Failed to get thread's name.");
			ArlithFrontend.getGuiLogger().err(e);
	...
	private void add(MessageValue msg) {
		try {
			...
			ArlithFrontend.getGuiLogger().err("An unexpected error occurred while trying to show a message that you sent.");
			throw new RuntimeException(e);
		}
	}
	...
	private @FXML void send() {
		...
		try {
			thread.sendMessageRequest(in).then(t -> {
				...
				ArlithFrontend.getGuiLogger().err("Failed to send your message.");
